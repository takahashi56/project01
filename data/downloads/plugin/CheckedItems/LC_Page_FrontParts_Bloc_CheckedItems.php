...
class LC_Page_FrontParts_Bloc_CheckedItems extends LC_Page_FrontParts_Bloc {
    ...
    function getItemList() {
        ...
        foreach ($arrItem as $name => $value) {
            ...
            $product_id = $arrRet['product_id'];
            $product_type = $objProduct->getProductsClassFullByProductId($product_id);
            $product_type_id = $product_type[0]['product_type_id'];

            if( $arrRet['product_id'] && $product_type_id == 2 ){
                // $arrRet += array('product_type_id'=>$product_type_id);
