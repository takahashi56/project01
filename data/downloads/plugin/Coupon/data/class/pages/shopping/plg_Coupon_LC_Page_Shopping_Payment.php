...
class plg_Coupon_LC_Page_Shopping_Payment extends LC_Page_Shopping_Payment_Ex {
		...
		switch ($this->getMode()) {
			case "confirm":
				...
				if (SC_Utils_Ex::isBlank($objPage->arrErr)) {
					...
					if($flg==false){
						...
						if(strpos($objPage->arrErr["coupon_id_name"],'ポイント付与') !== false){
							SC_Response_Ex::sendRedirect(SHOPPING_PAYMENT_URLPATH."?pt=".$objPage->arrErr["coupon_id_name"]);
						}
						else{
			...
			default:
				...
				if($objPage->arrForm["coupon_check"]["value"]==1){
					...
					if($_GET["pt"] != ""){
						$objPage->arrErr["coupon_id_name"] = htmlspecialchars($_GET["pt"]);
					}
				}
				break;
		}
		...
		if($coupon_check == '1' || (SC_MobileUserAgent::isMobile()&&strlen($coupn_id_name)>0) ) {
			...
			if(strlen($coupn_id_name)>0) {
				if( !$this->lfIsExistCoupon($coupn_id_name) ){
					...
					$where = "del_flg=0 AND coupon_id_name = ? AND discount_type = 2";
					$res = $objQuery->select("coupon_id,discount_price", "dtb_coupon", $where, array($coupn_id_name));
					...
					if($discount_price > 0){
						...
						$objQuery->update('dtb_customer', array('update_date' => 'CURRENT_TIMESTAMP'),'customer_id = ?', array($customer_id),array('point' => 'point + ?'), array($discount_price));

						$sqlVal = array();
						$sqlVal["coupon_used_id"] = $objQuery->nextVal("dtb_coupon_used_coupon_used_id");
						$sqlVal["coupon_id"]      = $coupon_id;
					...
					$objErr->arrErr["coupon_id_name"] = $discount_price."ポイント付与が完了いたしました。";
				}
			}
		}
	...
	function lfIsUsedCoupon($coupon_id_name) {
		...
		if($arrRet[0]['count_limit'] == 2){
			$count = $objQuery->count("dtb_coupon_used", "coupon_id=?", array($coupon_id));
			if($count > 0){
				return false;
			}
			else{
			...
			$customer_id = $_SESSION["customer"]["customer_id"];
			$count = $objQuery->count("dtb_coupon_used", "coupon_id=? AND customer_id=?", array($coupon_id,$customer_id));
			if($count > 0){
				return false;
			}
			else{
