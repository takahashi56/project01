...
class LC_Page_Shopping_Payment_Ex extends LC_Page_Shopping_Payment
{
    ...
    public function lfCheckError(&$objFormParam, $subtotal, $max_point)
    {
        ...
        if ($arrForm['point_check'] == '1'
            ...
            // ポイント購入の場合に不正にポイント使用を選択した場合
            if ($this->cartKey == PRODUCT_TYPE_NORMAL) {
              $objErr->arrErr['use_point'] = 'ポイントは使用できません。';
            } else {
              ...
              if ($arrForm['use_point'] > $max_point) {
                  $objErr->arrErr['use_point'] = '※ ご利用ポイントが所持ポイントを超えています。<br><a href="'.HTTPS_URL.'products/list.php?category_id=2" class="btn-style02 btn-elm btn-min">ポイントを購入する</a>';
              }
              if (($arrForm['use_point'] * POINT_VALUE) > $subtotal) {
                  $objErr->arrErr['use_point'] = '※ ご利用ポイントがご購入金額を超えています。<br>';
              }
              if ($arrForm['use_point'] < $subtotal) {
                  $objErr->arrErr['use_point'] = '※ ご利用ポイントがご購入金額を下回っています。<br>';
              }
              if ($arrForm['use_point'] > $subtotal) {
                  $objErr->arrErr['use_point'] = '※ ご利用ポイントがご購入金額を上回っています。<br>';
              }
