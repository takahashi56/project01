...
__EOS__;
        $where = ' T1.customer_id = ? AND T3.product_type_id = ? AND (T2.price = 0 OR (T1.payment_date IS NOT NULL AND ' . $dbFactory->getDownloadableDaysWhereSql('T1') . ' = 1))';
    ...
    public function strGetVideoUrl($filename, $base_path = 'http://xv.rchs-studio.com/v/', $time_limit = 86400) { // 24時間で時間切れ
        ...
        return $base_path . base64_encode($param);
    }
