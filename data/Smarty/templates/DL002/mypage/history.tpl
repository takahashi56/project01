        ...
        <div class="mycondition_area">
            <div class="panel panel-default">
                            ...
                            <div class="col-xs-9 col-sm-10 col-md-11">
                                ...
                                <!--{if $orderDetail.product_type_id == $smarty.const.PRODUCT_TYPE_DOWNLOAD}-->
                                    <!--{if $orderDetail.is_downloadable}-->
                                        <a target="_self" href="<!--{$smarty.const.ROOT_URLPATH}-->mypage/videos.php?order_id=<!--{$tpl_arrOrderData.order_id}-->&product_id=<!--{$orderDetail.product_id}-->&product_class_id=<!--{$orderDetail.product_class_id}-->" class="btn btn-default margin-sm">ビデオ一覧</a>
                                        ※ビデオ一覧から直接ご覧になれるようになりました。
                                        <br />
                                        <a target="_self" href="<!--{$orderDetail.down_videourl}-->?dl=1" class="btn btn-default margin-sm" download="<!--{$orderDetail.down_filename}-->">ダウンロード</a>
                                        ※ダウンロードできない場合は、右クリックして「名前を付けて保存」を選択してください。
                                        <!--{if $orderDetail.guide != ""}-->
                                        <br /><!--{$orderDetail.guide|nl2br_html}-->
                                        <!--{/if}-->
                ...
                <div class="panel-footer text-right">
                    ...
                    <!--{if $point_discount > 0}-->
                        <div><small>ポイント決済：</small>&minus;<!--{$point_discount|number_format}--> pt</div>
                    <!--{/if}-->
