...
<!-- contents -->
	...
	<!--{foreach from=$arrProducts item=arrProduct name=arrProducts}-->
		...
		<!--{if $smarty.foreach.arrProducts.first}-->
			...
			<!--▲ページナビ(上部)-->
			<!--▼商品wrap-->
			<h2 class="sp-tag list-title"><!--{$arrSearch.category|h}--></h2>
		...
		<div class=" clearfix product-list-item category-lists">
				...
				<div class="list-item-inner clearfix">
					...
					<!--{if $sub_images[$arrProduct.product_id]}-->
					<div class="sub-image pc-tag">
					<!--{foreach from=$sub_images[$arrProduct.product_id] item=sub}-->
						<div class="sub-wrap"><div class="product-list-img"><div>
							<a href="<!--{$smarty.const.P_DETAIL_URLPATH}--><!--{$arrProduct.product_id|u}-->"><img src="<!--{$smarty.const.IMAGE_SAVE_URLPATH}--><!--{$sub}-->"></a>
						</div></div></div>
					<!--{/foreach}-->
					</div>
					<!--{/if}-->
					...
					<div class="pointbox">
						<!--{strip}-->
						<!-- VIP会員価格がある場合は出力する -->
						<!--{if $rank_price[$arrProduct.product_id]}-->
							<img src="<!--{$TPL_URLPATH}-->img/icon/icon_price.png" />
							<span class="point">
							<!--{$rank_price[$arrProduct.product_id]|number_format}-->
