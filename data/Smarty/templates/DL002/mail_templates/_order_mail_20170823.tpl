...
<!--{if $arrOrder.payment_id ==  5}-->
お支払合計：<!--{$arrOrder.use_point*$smarty.const.POINT_VALUE+$arrOrder.discount|number_format|default:0}--> ポイント
<!--{else}-->
...
単価：<!--{$arrOrderDetail[cnt].price|sfCalcIncTax:$arrOrderDetail[cnt].tax_rate:$arrOrderDetail[cnt].tax_rule|number_format}--> ポイント
数量：<!--{$arrOrderDetail[cnt].quantity}-->

<!--{/section}-->
-------------------------------------------------
小　計 <!--{$arrOrder.subtotal|number_format|default:0}--> ポイント

手数料 <!--{$arrOrder.charge|number_format|default:0}--> ポイント
============================================
<!--{if $arrOrder.use_point > 0}-->
使用ポイント <!--{$arrOrder.use_point*$smarty.const.POINT_VALUE+$arrOrder.discount|number_format|default:0}--> ポイント
<!--{/if}-->
...
商品コード: <!--{$arrOrderDetail[cnt].product_code}-->
商品名: <!--{$arrOrderDetail[cnt].product_name}--> <!--{$arrOrderDetail[cnt].classcategory_name1}--> <!--{$arrOrderDetail[cnt].classcategory_name2}-->
単価：<!--{$arrOrderDetail[cnt].price|sfCalcIncTax:$arrOrderDetail[cnt].tax_rate:$arrOrderDetail[cnt].tax_rule|number_format}--> 円
数量：<!--{$arrOrderDetail[cnt].quantity}-->

<!--{/section}-->
-------------------------------------------------
小　計 <!--{$arrOrder.subtotal|number_format|default:0}--> 円 <!--{if 0 < $arrOrder.tax}-->(うち消費税 <!--{$arrOrder.tax|number_format|default:0}--> 円)<!--{/if}-->

<!--{if $arrOrder.use_point > 0}-->
値引き <!--{$arrOrder.use_point*$smarty.const.POINT_VALUE+$arrOrder.discount|number_format|default:0}--> 円
<!--{/if}-->
