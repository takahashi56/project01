...
__EOS__;
    ...
    public function getProductStatus($productIds)
    {
        ...
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $cols = 'd.product_id, d.product_status_id, m.name AS icon';
        $from = 'dtb_product_status AS d, mtb_status_image AS m';
        $where = 'del_flg = 0 AND (d.product_status_id=m.id) AND product_id IN (' . SC_Utils_Ex::repeatStrWithSeparator('?', count($productIds)) . ')';
    ...
    /**
     * 商品IDをキーにした, 商品ステータスアイコンの配列を取得する.
     *
     * @param array 商品ID の配列
     * @return array 商品IDをキーにした商品ステータスIDの配列
     */
    public function getProductStatusIcon($productIds)
    {
        if (empty($productIds)) {
            return array();
        }
        ...
        $results = array();
        foreach ($productStatus as $status) {
            $results[$status['product_id']][] = $status['icon'];
        }
