...
class SC_Helper_Purchase
{
    ...
    public function registerOrderComplete($orderParams, &$objCartSession, $cartKey)
    {
        ...
        $objQuery->update('dtb_order_temp', array('del_flg' => 0),
                          'order_temp_id = ?',
...
__EOS__;
    ...
    /**
     * クレジットカード決済済みで未発送の物を取得
     */
    public function getCompleatedCreditcardList()
    {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $res = $objQuery->select('order_id, add_point', 'dtb_order as o', 'order_id > 1300 AND payment_id = 6 AND status=6');
        return $res;
    }
    
    /**
     * クレジットカード発送済みで未発送の物を発送済みにする
     */
    public function setCompleatedCreditcardList()
    {
      $arrOrders = $this->getCompleatedCreditcardList();
      foreach($arrOrders as $order) {
        $this->sfUpdateOrderStatus($order['order_id'], 5,
                                       $order['add_point']);
      }
    }
}
